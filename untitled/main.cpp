...
struct Node *process(struct Node *root) {
    ...
    if (left == nullptr &&
        right == nullptr) {                //if leaf node actual will simply be the 1st available pos on its level
        actual_pos = next_pos[height];
        modifier = 0;
    } else if (left == nullptr || right == nullptr) {            //only one child
        if (left == nullptr) {                            //only right child
            right = process(right);
            prov_pos = right->x - 1;
